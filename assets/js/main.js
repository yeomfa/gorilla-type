...
const init = function() {
  ...
  quoteEl.textContent = 'A good day to learn...(Ctrl + Enter)';
...
const startType = function() {
  ...
  // Display quote 
  const num = Math.trunc(Math.random() * englishQuotes.length);
  displayText(englishQuotes[num]); 
...
document.addEventListener('keydown', (e) => {
  let key = e.key;
  const isLegalKey = !excludes.find(element => element === key);
  if (isStart && isLegalKey) {
    if (key === ' ') {
      e.preventDefault(); 
      validateTyped(key);
    } else if (key === 'Dead') {
...
document.addEventListener('keydown', (e) => {
  ...
  if ( ctrlKey && key === 'Enter') {
    ...
    e.preventDefault();
  } else if (!btnReset.classList.contains('hidden') && key === 'Tab') {
    e.preventDefault();
    btnReset.focus();
  }
});
